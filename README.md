...
>
> При операции `-15 / 2` в обычном делении получится `-7.5`, но оператор `//` отбрасывает дробную часть и округляет
> результат вниз до ближайшего целого числа, в нашем случае до `-8`.
...
### Таблица арифметических операторов Python
...
Попробуйте экспериментировать с различными операциями, чтобы лучше усвоить материал.
...
Python эта задача решается автоматически функцией `print()`.
...
Не бойтесь экспериментировать, пробуйте выводить различные типы данных, даже целые стихотворения.
...
ИИзменим его, чтобы вместо символа перевода строки `\n` в конце был другой символ, который мы сможем увидеть.
...
Функция `print()` вызывается с аргументами `10` и `20`. Для параметра `sep` передается аргумент `"!"`, который
